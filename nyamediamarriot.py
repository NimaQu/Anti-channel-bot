...
async def handle_reservation(client: Client, message: Message):
    ...
    if len(args) < 3:
        await message.reply('亲亲请按照 /reserve [入住日期] [入住天数] [信用卡卡号] [姓名] [过期时间] [账单地址] [cvv] 的格式预定哦')
        return
    if len(args) > 3:
        if args[3] is not None and args[3].startswith('37'):
            await message.reply('亲亲 no AMEX 哦')
            return
...
async def handle_checkout(client: Client, message: Message):
    ...
    if room_number is None:
        await message.reply('亲亲找不到您的房间信息哦')
        return
